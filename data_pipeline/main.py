...
def scrape_books(pages=5):
    ...
    for page in range(1,pages+1):
        ...
        for book in soup.find_all('article', class_='product_pod'):
            ...
            try:
                prod_soup=BeautifulSoup(requests.get(BASE_URL+rel, headers=headers, timeout=10).text,'html.parser')
                category=prod_soup.select('ul.breadcrumb li a')[2].text.strip()
                time.sleep(0.15)
            except:
                category="Default"
...
print("--- STEP 1: SCRAPE ---")
df=scrape_books(5)
print(f"Total {len(df)} rows, categories: {df['category'].nunique()} unique")
print("\n--- STEP 2-3: CLEAN ---")
...
queries={"q1_SELECT_WHERE":"SELECT title,price_inr FROM books WHERE rating>=4","q2_ORDER_BY":"SELECT title,price_gbp FROM books ORDER BY price_gbp DESC","q3_LIMIT":"SELECT title FROM books LIMIT 10","q4_DISTINCT":"SELECT DISTINCT rating FROM books","q5_IN_BETWEEN_JOIN":"SELECT c.category_name,b.title,b.rating,b.price_inr FROM books b JOIN categories c ON b.category_id=c.category_id WHERE b.rating IN (4,5) AND b.price_inr BETWEEN 2000 AND 6000 ORDER BY b.rating DESC LIMIT 10"}
for name,sql in queries.items():
    print(f"\n-- {name} --"); print(pd.read_sql(sql, conn).head())
print("\n--- STEP 6: VERIFICATION ---")
sql_join="SELECT c.category_name,b.title,b.rating,b.price_inr FROM books b JOIN categories c ON b.category_id=c.category_id ORDER BY b.rating DESC LIMIT 5"
...
df_merge=pd.merge(df_books, df_cats, on="category_id")[["category_name","title","rating","price_inr"]].sort_values("rating",ascending=False).head(5)
print(df_sql); print(df_merge); print(f"Equivalent: {df_sql.shape==df_merge.shape}")
conn.close()
